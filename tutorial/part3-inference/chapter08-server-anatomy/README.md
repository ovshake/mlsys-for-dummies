...
async def generate(self, request):
    ...
    # Send to scheduler via IPC (ZeroMQ in SGLang)
    await self.send_to_scheduler(input_ids, request.params)

    # Wait for responses
    async for token in self.receive_tokens():
        yield token
```
...
def get_next_batch(self):
    # Priority: prefill new requests OR continue decode
    if self.should_prefill():
        batch = self.build_prefill_batch()
    else:
        batch = self.build_decode_batch()
    return batch
```
...
- Original source: [`sglang/code-walk-through/readme.md`](../../../sglang/code-walk-through/readme.md)
